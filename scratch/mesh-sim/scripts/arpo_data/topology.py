...
## @brief Resolve a MAC address to its ``rabN.M`` radio label.
#
# Returns ``None`` if no rab ever claimed this MAC as a local MAC, meaning it
# is a peer-only address that cannot be tied to a specific radio.
#
# @param mac      MAC address string (colon-separated hex).
# @param csv_root CSV root to scan; defaults to @ref CSV_ROOT when ``None``.
# @return ``"rabN.M"`` label or ``None``.
